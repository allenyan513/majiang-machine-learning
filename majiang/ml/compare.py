...
def _worker(args: tuple[str, str, list[int]]) -> np.ndarray:
    name, opponent, seeds = args
...
def compare(candidates: list[str], n: int, seed: int = 0, workers: int | None = None,
            opponent: str = "rule") -> dict[str, np.ndarray]:
    ...
    with Pool(workers) as pool:
        for name in candidates:
            parts = pool.map(_worker, [(name, opponent, c) for c in chunks if c])
...
def report(results: dict[str, np.ndarray], opponent: str = "rule") -> None:
    names = list(results)
    base = names[0]
    n = len(results[base])
...
def main() -> None:
    ...
    ap.add_argument("--opponent", default="rule", help="另外三个座位用谁（rule / rule_v1 / nn:...）")
    a = ap.parse_args()
    report(compare(a.candidates.split(","), a.n, a.seed, a.workers, a.opponent), a.opponent)
