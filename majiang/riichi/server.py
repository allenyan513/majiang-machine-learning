"""日本立直麻将单机游戏服务器（标准库，无额外依赖）。教程主线用的是简化规则，这个是独立保留的。

    uv run python -m majiang.riichi.server --port 8001
    然后打开 http://localhost:8001

路由：
    /            牌桌（web/riichi/index.html）
    /api/...     引擎
...
from majiang.riichi.shanten import discard_options, shanten
from majiang.riichi.tile import WINDS, dora_from_indicator

WEB_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "web", "riichi")


class Session:
    def __init__(self, agent_names: list[str], model_path: str | None = None):
        self.model = None  # 日麻还没有训练过的模型
...
def main() -> None:
    ...
    args = ap.parse_args()
    session = Session(args.agents.split(","))
    session.new(0)
    srv = ThreadingHTTPServer((args.host, args.port), make_handler(session))
    print(f"日麻牌桌 http://localhost:{args.port}/   agents={','.join(session.agent_names)}", flush=True)
    srv.serve_forever()
