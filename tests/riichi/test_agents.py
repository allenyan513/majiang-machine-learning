...
from majiang.riichi.actions import ActionType, discard, riichi
from majiang.riichi.game import Game, Phase
from majiang.riichi.tile import parse_hand
from tests.riichi.test_game import FILLER, _rig
...
def test_rule_agent_beats_random_and_never_illegal():
    ...
    wins = 0
    for s in range(30):
        r = play_game([RuleAgent(), RandomAgent(s), RandomAgent(s + 1), RandomAgent(s + 2)], seed=s)
        wins += any(w.player == 0 for w in r.wins)
    assert wins / 30 > 0.4
